...
public class Mario extends Player{
	...
	public void tick(){
		...
		if(!grabbed && !dead) {
			super.tick();
			if (!this.hit) {
				if (handler.getKeyManager().keyJustPressed(KeyEvent.VK_SPACE) && !handler.getKeyManager().up && !handler.getKeyManager().down && canMove) {
					this.jump();
				}

				if (handler.getKeyManager().right && !handler.getKeyManager().up && !handler.getKeyManager().down && canMove) {
					...
					facing = "Right";
					moving = true;
				} else if (handler.getKeyManager().left && !handler.getKeyManager().up && !handler.getKeyManager().down && canMove) {
					if (handler.getKeyManager().runbutt) {
						velX = -6;
						running = true;
					} else {
